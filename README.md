...
应用内「接口与外观设置」的「正文比例」可分别调整字号（16–28 sp）、行距（125%–200%）、左右边距（12–44 dp）与段落间距（12–72 dp），「参考图」可一键恢复本次截图使用的组合。句子浮层英文原句随字号按 1.12 倍显示。界面顶部额外留出约 0.8 行正文高度；文章列表可从正文横向拖出。正文标题采用 small caps，完整显示；目录标题最多显示两行。设置页全屏磨砂、底部固定保存，可手动检查 GitHub 最新正式版并前往下载。「正文标记已查询词」开关默认关闭，不影响查询记录或缓存。右上角百分比显示当前文章的句子缓存进度，点击可查看推理状态与已缓存的原句/译文。
